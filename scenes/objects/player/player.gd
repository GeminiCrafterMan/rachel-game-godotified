...
func _input(event):
	if event is InputEventMouseMotion and Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
		...
		$GunRay.rotate_x(-event.relative.y * MOUSE_SENSITIVITY)
		$GunRay.rotation.z = clampf($GunRay.rotation.z, -deg_to_rad(90), deg_to_rad(90))
		...
		$Cameras/ThirdPersonCamera.rotate_x(-event.relative.y * MOUSE_SENSITIVITY)
		$Cameras/ThirdPersonCamera.rotation.x = clampf($Cameras/ThirdPersonCamera.rotation.x, -deg_to_rad(90), deg_to_rad(90))
